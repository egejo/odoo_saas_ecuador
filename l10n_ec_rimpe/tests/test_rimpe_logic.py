...
class TestRimpeLogic(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.partner_popular = cls.Partner.create(
            {"name": "Tienda Popular", "l10n_ec_rimpe_type": "popular_business"}
        )

        cls.partner_entrepreneur = cls.Partner.create(
            {"name": "Startup Ecuador", "l10n_ec_rimpe_type": "entrepreneur"}
        )

        cls.partner_none = cls.Partner.create(
            {"name": "Empresa Grande", "l10n_ec_rimpe_type": "none"}
        )
